...
const anthropicEnvSchema = z.object({
  ...
  ANTHROPIC_MAX_TOKENS: z.coerce.number().int().positive().default(32000),
});
...
function buildTlsConfig(config: TemporalRuntimeConfig): ConnectionOptions["tls"] {
...
export function getTemporalNativeConnectionOptions(): NativeConnectionOptions {
  const config = getTemporalRuntimeConfig();
