...
export function buildProjectNameFromPrompt(prompt: string, fallbackName: string): string {
  const tokens = prompt
    .trim()
    .replace(/\s+/g, " ")
    .split(" ")
...
export function createInitialBuildPlan(prompt: string, projectName: string): InitialBuildPlan {
  const normalizedPrompt = prompt.trim().replace(/\s+/g, " ");
  const rawTokens = normalizedPrompt.toLowerCase().match(/[a-z0-9-]+/g) ?? [];
  const keywords = Array.from(
    new Set(
      rawTokens.filter((token) => token.length > 2 && !promptStopWords.has(token)),
    ),
  ).slice(0, 8);
