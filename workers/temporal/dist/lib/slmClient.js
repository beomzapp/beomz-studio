...
export function keywordPaletteFallback(prompt) {
...
// ─── HTTP helper ───────────────────────────────────────────────────────────
async function slmPost(path, body) {
...
export async function matchTemplateWithSlm(input) {
    ...
    const validIds = new Set(definitions.map((t) => t.id));
    ...
    try {
        ...
        const ranked = await slmPost("/match-template", { prompt: augmentedPrompt, templates: allTemplates });
        ...
        const scores = ranked.reduce((acc, r) => {
 ...
 */
export async function classifyPaletteWithSlm(prompt, templateId = "") {
    try {
        return await slmPost("/classify-palette", {
            prompt,
